...
/* This plugin writes GMV format video data files using Random Access Method:
	...
	Algirdas Toleikis, modified March, 2013. 
		add_info==1 -> stageXnm, stageYnm, focusZnm
...
// get  or generate frame specific information from the GMV header

	sliceLabel= " " + stack.getSliceLabel(slice);	// avoid "null" string error by adding a space!

	if (sliceLabel.indexOf("GMV") <1){

		fr_time = (int) (exp_ms * (slice-1)); // best guess at individual frame times

	} else {
	
	if (slice == 1) {
		startTime = Integer.parseInt (keyVal(sliceLabel, "fr_time"));
	}

	leftx=			Integer.parseInt (keyVal(sliceLabel, "leftx"));
	width=			Integer.parseInt (keyVal(sliceLabel, "width"));
	topy=			Integer.parseInt (keyVal(sliceLabel, "topy"));
	height=		Integer.parseInt (keyVal(sliceLabel, "height"));
	exp_ms= 		Integer.parseInt (keyVal(sliceLabel, "exp_ms"));
	fr_size=		Integer.parseInt (keyVal(sliceLabel, "fr_size"));
	fr_time=		Integer.parseInt (keyVal(sliceLabel, "fr_time")) - startTime;

	x_nm_pixels=		Float.valueOf (keyVal(sliceLabel, "x_nm_pixels"));
	y_nm_pixels=		Float.valueOf (keyVal(sliceLabel, "y_nm_pixels"));

	igain =			(byte) (Integer.parseInt(keyVal(sliceLabel, "igain"))) ;
	vgain =		(byte) (Integer.parseInt(keyVal(sliceLabel, "vgain"))) ;
	bit_pix =		(byte) (Integer.parseInt(keyVal(sliceLabel, "bit_pix"))) ;
	bin =			(byte) (Integer.parseInt(keyVal(sliceLabel, "bin"))) ;
	byte_info =		(byte) (Integer.parseInt(keyVal(sliceLabel, "byte_info"))) ;


	add_info =		(byte) (Integer.parseInt (keyVal(sliceLabel, "add_info")));

	laser_power = 	Short.parseShort (keyVal(sliceLabel, "laser_power"));
	temperature = 	Short.parseShort (keyVal(sliceLabel, "temperature"));
	illum_time = 		Short.parseShort (keyVal(sliceLabel, "illum_time"));
	
	if (add_info==1)  {
	stageXnm =		Integer.parseInt (keyVal(sliceLabel, "stageXnm"));
	stageYnm =		Integer.parseInt (keyVal(sliceLabel, "stageYnm"));
	focusZnm = 		Integer.parseInt (keyVal(sliceLabel, "focusZnm"));

	dummy_int1 =	Integer.parseInt (keyVal(sliceLabel, "int1"));
	dummy_int2 =	Integer.parseInt (keyVal(sliceLabel, "int2"));
	dummy_int3 =	Integer.parseInt (keyVal(sliceLabel, "int3"));

	dummy_float1 =	Float.valueOf (keyVal(sliceLabel, "float1"));
	dummy_float2 =	Float.valueOf (keyVal(sliceLabel, "float2"));
	dummy_float3 =	Float.valueOf (keyVal(sliceLabel, "float3"));
	dummy_float4 =	Float.valueOf (keyVal(sliceLabel, "float4"));
	}	
...
// ======================= string manipulator  ==================
 public String  keyVal (String instr, String Qstr)
  {

// format of the info header is  "keyword: value" + carriage return
// first find the Query-keyword, then find the ":", then the line terminator

	int st = instr.indexOf(Qstr,0);
	int eq = instr.indexOf(":",st);
	int nd = instr.indexOf("\n", eq);

	if ( (st<0) || (eq<0)) {
		return "0";
	} else if  (nd<0) {
		return   instr.substring (eq+2);
   	} else {
		return instr.substring ( eq+2, nd );
	}  
}
