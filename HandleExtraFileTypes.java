...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
	...
	private Object tryOpen(String directory, String name, String path) {
		...
		// check if the file ends in .GMR
		if (name.endsWith(".gmr")) {
			return tryPlugIn("GMV_Reader", path);
		}

		// check if the file ends in .GMG
		if (name.endsWith(".gmg")) {
			return tryPlugIn("GMV_Reader", path);
		}

		if (name.endsWith(".mvw")) {
			return tryPlugIn("MVW_Reader", path);
		}
